...
class DataValSet(data.Dataset):
    def __init__(self, root_dir):
        ...
        self.input_dir = join(os.path.dirname(root_dir), 'lr')
        self.sr_dir = join(os.path.dirname(root_dir), 'hr')

        if not os.path.exists(self.input_dir):
            os.makedirs(self.input_dir)

        if not os.path.exists(self.sr_dir):
            os.makedirs(self.sr_dir)

        shutil.copyfile(root_dir, join(self.input_dir, os.path.basename(root_dir)))
        shutil.copyfile(root_dir, join(self.sr_dir, os.path.basename(root_dir)))
